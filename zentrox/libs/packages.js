...
function listPackages(callback) {
  ...
  i = 0;
  if (supported_os) {
    data = chpr.execSync(listCommand, {
      encoding: "utf-8",
...
function listInstalledPackages() {
  ...
  i = 0;
  if (supported_os) {
    data = chpr.execSync(listInstalledCommand, {
      encoding: "utf-8",
      maxBuffer: 1024 * 1024 * 1024,
    });
...
function installPackage(name, password) {
  ...
  if (data.includes("Complete!") || data.split("\n").length > 1) {
    return true;
  } else {
    return false;
  }
...
function removePackage(name, password) {
  try {
    data = chpr
      .execSync(
        `echo "${password}" | sudo -S ` + removeCommand.replace("PCKG", name),
      )
      .toString("utf-8");
  } catch {
    return false;
