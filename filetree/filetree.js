      ...
      if (fs.statSync(path.join(location, file)).isFile()) {
        ...
        if (file[0] == "." || file[0] == "$") {
          s =
            ...
            file +
            '" oncontextmenu=makeContext("' +
            encodeURIComponent(file) +
            '") onclick=handleFile("' +
            encodeURIComponent(file) +
            '") style=opacity:70%><img src="' +
            img_path +
            '" height=16> ' +
            file +
            "</button><br>";
...
function renderContentView(location) {
  ...
  if (!canRW) {
    ...
    document.getElementById("pathInput").title =
      "You can not write in this folder";
  } else {
      ...
      if (fs.statSync(path.join(location, file)).isFile()) {
        try {
          if (extensionSort[path.basename(file).split(".")[1]] != "image") {
            img_path =
              "../images/file_icons/" +
              extensionSort[path.basename(file).split(".")[1]] +
              ".png";
        ...
        if (file[0] == "." || file[0] == "$") {
          if (localStorage.getItem("showIddenFiles") == "true") {
            ...
            s =
              ...
              '")><img src="' +
              img_path +
              "\" onerror=this.src='../images/file_icons/image.png' height=16>" +
              file +
              "</button>";
          }
        } else {
...
function showPreview(name) {
  if (
    extensionSort[path.basename(name).split(".")[1]] == "code" &&
    fs.statSync(path.join(currentLocation, name)).size < 1024 * 1024
  ) {
    document.getElementById("browser_window").style.width =
      "calc(70vw - 300px)";
    document.getElementById("preview_window").hidden = false;
    if (cachedSyntax[name] == null) {
      highlighted_string =
        "<pre><code style=font-size:large;word-wrap:break-word;>" +
        hljs.highlightAuto(
          fs
            .readFileSync(path.join(currentLocation, name))
            .toString()
            .replace("\n", "\n\r")
        ).value +
        "</code></pre>";
    ...
    document.getElementById("preview_window").innerHTML =
      "File is to big for preview";
  } else if (
    extensionSort[path.basename(name).split(".")[1]] == "image" &&
    path.basename(name).split(".")[1] != "gif"
  ) {
    document.getElementById("browser_window").style.width =
      "calc(70vw - 300px)";
    document.getElementById("preview_window").hidden = false;
    document.getElementById("preview_window").innerHTML =
      '<img src="' +
      "data:image/" +
      path.basename(name).split(".")[1] +
      ";base64," +
      fs.readFileSync(path.join(currentLocation, name)).toString("base64") +
      '" alt="' +
      name +
      "\" onerror=this.src='../images/file_icons/image.png'>";
  } else if (path.basename(name).split(".")[1] == "md") {
    document.getElementById("browser_window").style.width =
      "calc(70vw - 300px)";
    document.getElementById("preview_window").hidden = false;
    var parser = new DOMParser();
    var par_obj = parser.parseFromString(
      fs.readFileSync(path.join(currentLocation, name)).toString(),
      "text/html"
    );
    ...
    document.getElementById("preview_window").innerHTML = marked.marked(
      html_code
        .replace(bad_link_detect, "$1")
        .replace(to_dir_link, "[$1](javascript:goToNote('$1'))")
    );
    ...
    for (e of document.querySelectorAll("#preview_window img")) {
      try {
        e.src =
          ...
          fs
            .readFileSync(
              path.join(currentLocation, e.attributes.src.nodeValue)
            )
            .toString("base64");
      } catch {
        e.remove();
  ...
  if (type == "file") {
    if (!isEncryptedFile(path.join(currentLocation, filename))) {
      ...
      inputModal(function () {
        ...
        try {
          if (
            fs.statSync(path.join(currentLocation, filename)).size <
            1024 * 1204 * 5
          ) {
            ccode = decrypt(
              JSON.parse(
                fs
                  .readFileSync(path.join(currentLocation, filename), {
                    encoding: "binary",
                  })
                  .split("\nNOHEAD\n")[1]
              ),
            ...
            fs.writeFileSync(
              "decryptedDumpTest." +
                path.basename(path.join(currentLocation, filename)).split(".")[
                  path.basename(path.join(currentLocation, filename)).split(".")
                    .length - 1
                ],
              ccode,
            ...
            errorModal(
              "Encryption closing",
              "Please close the file by clicking Ok, when you've viewed it.",
              function () {
                setTimeout(function () {
                  fs.writeFileSync(
                    "decryptedDumpTest." +
                      path
                        .basename(path.join(currentLocation, filename))
                        .split(".")[
                        path
                          .basename(path.join(currentLocation, filename))
                          .split(".").length - 1
            ...
            errorModal(
              "Encrypted file",
              "This file is encrypted and too large for direct decryption.\nPlease decrypt it manualy and then try again.\nYou can also right click and use open to view the encrypted data.",
              function () {}
            );
          }
        } catch (e) {
    ...
    errorModal(
      "Error",
      "This file can not be opened.<br>Please try again as admin/sudo.",
      function () {}
    );
  }
} // ? Manage a file click
...
function inputModal(
  ...
  document.getElementById("inputModalInput").onkeydown = function (e) {
    if (e.key == "Enter") {
      ...
      setTimeout(function () {
        (document.getElementById("inputModalInput").value = ""),
          (document.getElementById("inputModalInput").placeholder = "");
      }, 600);
      listener();
  ...
  for (e of dropdownList) {
    dropdownHTML +=
      "<button onclick=insertDropdownSelection('" +
...
function pasteFile() {
  ...
  if (fs.statSync(file_in_copy).isFile()) {
    if (
      fs.existsSync(path.join(currentLocation, path.basename(file_in_copy)))
    ) {
      errorModal(
        "Paste error",
        "In this folder already is a file called " +
          path.basename(file_in_copy),
        function () {
          inputModal(function () {
            fs.copyFileSync(
              file_in_copy,
...
function moveFile() {
  ...
  if (fs.statSync(file_in_move).isFile()) {
    if (
      fs.existsSync(path.join(currentLocation, path.basename(file_in_move)))
    ) {
      errorModal(
        "Paste error",
        "In this folder already is a file called " +
          path.basename(file_in_move),
        function () {
          inputModal(function () {
            fs.copyFileSync(
              file_in_move,
      ...
      fs.copyFileSync(
        file_in_move,
        path.join(currentLocation, path.basename(file_in_move))
      );
      renderContentView(currentLocation);
      renderFolderList(currentLocation);
      fs.unlinkSync(file_in_move);
...
function commandLine() {
  ...
  inputModal(
    function () {
      ...
      if (
        newPathFromInput.match(".* from .*") != null &&
        newPathFromInput.split(" from ")[1] != null
      ) {
        fs.writeFileSync(
          locationused + newPathFromInput.split(" from ")[0],
          fs.readFileSync(
            path.join(locationused, newPathFromInput.split(" from ")[1]),
            {
              encoding: "utf-8",
            }
          )
        ...
        newPathFromInput.match("[$]rm .*[.].*") != null &&
        newPathFromInput[0] == "$"
      ) {
        errorModal(
          "Delete file",
          "Do you want to remove the file " +
            newPathFromInput.split("remove ")[1] +
            "?",
          function () {
            fs.unlinkSync(
              path.join(locationused, newPathFromInput.split("remove ")[1])
            );
            renderContentView(currentLocation);
            renderFolderList(currentLocation);
          }
        ...
        newPathFromInput.match("^[$]find ") != null &&
        newPathFromInput[0] == "$"
      ) {
        file_list = [];
        folder_list = [];
        ...
        if (newPathFromInput.split(" ")[1] != null) {
          try {
            navigator.clipboard.writeText(
              "data:" +
                require("mime-types").lookup(
                  newPathFromInput.split(" ")[1].split(".")[1]
                ) +
                ";base64" +
                "," +
                fs
                  .readFileSync(
                    path.join(currentLocation, newPathFromInput.split(" ")[1])
                  )
                  .toString("base64")
            );
            confirmModal(
            ...
            errorModal(
              "Base64",
              "Can't generate base64. Wrong filename.",
              function () {}
            );
            console.error(e);
          }
          ...
          confirmModal(
            "Base64",
            "Please enter a filename to get a base64 value of."
          );
        }
      } else if (newPathFromInput.split(" ")[0] == "$compress") {
        if (newPathFromInput.split(" ")[1] != null) {
          context_file = path.join(
            currentLocation,
            newPathFromInput.split(" ")[1]
          );
        ...
        if (newPathFromInput.split(" ")[1] != null) {
          try {
            ...
            if (!isNaN(needed_i)) {
              ...
              fs.writeFileSync(
                path.join(currentLocation, "random_file.txt"),
                string
              );
              renderContentView(currentLocation);
              renderFolderList(currentLocation);
              ...
              errorModal(
                "Generation failed",
                "Please enter a number, on how many bytes you want to generate.",
                function () {}
              );
            }
          } catch (e) {
        ...
        !fs.existsSync(locationused + newPathFromInput) ||
        newPathFromInput.match("^[$]new ") != null
      ) {
...
function isEncryptedFile(filepath) {
  if (fs.existsSync(filepath) && fs.statSync(filepath).isFile()) {
    ...
    console.warn(
      "The file you tried to load doesn't exist. (" +
        filepath +
        "). isEncryptedFile()"
    );
    return false;
...
function showDetails() {
  ...
  document.getElementById("details_filename").innerText =
    path.basename(context_file);
  extension = "";
  ...
  try {
    if (fs.statSync(context_file).isFile()) {
      extension = path
        ...
        .split(".")
        [path.basename(context_file).split(".").length - 1].toUpperCase();
      size =
        "~" +
        Math.round(fs.statSync(context_file).size / (1024 * 1024)) +
        "MB" +
        " (" +
        fs.statSync(context_file).size +
        "B)";
    ...
    document.getElementById("details_encrypted").innerHTML =
      "Encrypted: " + encrypted;
    document.getElementById("details_owner").innerHTML =
      "Owner UID: " + fs.statSync(context_file).uid;
  } catch (e) {
    console.error(e);
    terminateDetails();
...
function encryptionManager() {
  ...
  if (isEncryptedFile(context_file)) {
    ...
    document.getElementById("encryption_button").innerText =
      "Remove encryption";
  } else {
...
function encryptionSetup() {
  if (isEncryptedFile(context_file)) {
    ...
    confirmModal(
      "Remove encryption",
      "Do you want to remove the encryption from this file?",
      function () {
        inputModal(function () {
          try {
            ccode = decrypt(
    ...
    confirmModal(
      "Apply encryption",
      "When the encryption is applied, you need a password to read and edit the file.\nIt uses AES256. I would recommend choosing a strong password.\nYou're editing the file " +
        context_file,
      function () {
        inputModal(function () {
          try {
            ccode = encrypt(
              fs.readFileSync(context_file, {
                encoding: "binary",
              }),
              document.getElementById("inputModalInput").value
            );
            ...
            errorModal(
              "Encryption failed",
              "Password was too long or short\nIt has to be 32 chars long",
              function () {}
            );
            console.error(e);
          }
...
function compressFile() {
  ...
  if (fs.statSync(context_file).isFile()) {
    ...
    wstream = zip_e.generateNodeStream({
      type: "nodebuffer",
      streamFiles: true,
      compression: "DEFLATE",
    });
    wstream
      .pipe(
        fs.createWriteStream(
          path.join(
            currentLocation,
            document.getElementById("gzipOutputName").value + ".zip"
          )
        )
      )
      .once("close", function () {
    ...
    confirmModal(
      "Compression failed",
      "Folder compression isn't supported right now.",
      function () {}
    );
  }
}
...
window.addEventListener("load", function () {
  ...
  this.document.getElementById("pathInput").onkeydown = function (e) {
    if (e.key == "Enter") {
      if (this.value[0] != "?") renderFolderList(this.value);
  ...
  this.document.getElementById("contextmenu_delete").onclick = function () {
    confirmModalWarning(
      ...
      `Do you want to remove <b>${
        context_file.replace("/", "\\").split("\\")[
          context_file.replace("/", "\\").split("\\").length - 1
        ]
      }</b>?`,
      function () {
        try {
  ...
  this.document.getElementById("contextmenu_copy").onclick = function () {
    ...
    document.getElementById("path_action_indicator").innerHTML =
      "<img src=../images/times.png onclick=terminateCopy()> Copy " +
      file_in_copy +
  ...
  this.document.getElementById("contextmenu_move").onclick = function () {
    ...
    document.getElementById("path_action_indicator").innerHTML =
      "<img src=../images/times.png onclick=terminatemove()> Copy " +
      file_in_move +
      " <button onclick=moveFile()>Move</button>";
    document.getElementById("path_action_indicator").hidden = false;
  };
  this.document.getElementById("contextmenu_new").onclick = function () {
    commandLine();
  };
  this.document.getElementById("contextmenu_details").onclick = function () {
    showDetails();
  };
...
window.addEventListener("keydown", function (e) {
  if (e.key == "Escape") {
    ...
    if (
      selected_file_id < 0 ||
      this.document.querySelectorAll("#browser_window button").length <
        selected_file_id
    ) {
      selected_file_id = 0;
    }
    ...
    this.document.querySelector(
      "#browser_window button:nth-child(" + selected_file_id + ")"
    ).style.background = "#0570db";
    this.document
      .querySelector(
        "#browser_window button:nth-child(" + selected_file_id + ")"
      )
